const data = [
    ...
    let i = 0;
    window.onload = dataRender = () => {
        i++  
        const quote = data[i]
        output.innerHTML = ""
        output.innerHTML += `<h1>${quote.quoteText}</h1>`
        output.innerHTML += `<h3>${quote.quoteAuthor}</h3>`
    }

    setInterval(dataRender, 1000)

    button.addEventListener("click", dataRender)
